...
contract CMETA is FunctionsClient, ERC20, Pausable {
    ...
    function sendRedeemRequest(uint256 amount) external whenNotPaused returns (bytes32 requestId) {
        uint256 amountTslaInUsdc = getUsdcValueOfUsd(getUsdValueOfMeta(amount));
    ...
    function _mintFulFillRequest(bytes32 requestId, bytes memory response) internal {
        ...
        if (amountOfTokensToMint != 0) {
            uint256 tokensInUSDC = getUSDCValueofUSD(getUsdValueOfMeta(amountOfTokensToMint));
    ...
    function _redeemFulFillRequest(bytes32 requestId, bytes memory response) internal {
        ...
        if (amountTokensWithdraw != 0) {
            uint256 tokensInUSDC = getUSDCValueofUSD(getUsdValueOfMeta(amountTokensWithdraw));
    ...
    function getMetaPrice() public view returns (uint256) {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(i_metaUsdFeed);
    ...
    function getUsdValueOfMeta(uint256 tslaAmount) public view returns (uint256) {
        return (tslaAmount * getMetaPrice()) / PRECISION;
    }
    ...
    function getTotalUsdValue() public view returns (uint256) {
        return (totalSupply() * getMetaPrice()) / PRECISION;
    }

    function getCalculatedNewTotalValue(uint256 addedNumberOfTsla) public view returns (uint256) {
        return ((totalSupply() + addedNumberOfTsla) * getMetaPrice()) / PRECISION;
    }
