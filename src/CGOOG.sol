...
contract CGOOG is FunctionsClient, ERC20, Pausable {
    ...
    constructor(
        ...
        ERC20("Google Token", "CGOOG")
    {
        owner = msg.sender;
        s_mintSource = mintSource;
        s_redeemSource = redeemSource;
        s_donID = donId;
        i_googUsdFeed = googPriceFeed;
    ...
    function sendRedeemRequest(uint256 amount) external whenNotPaused returns (bytes32 requestId) {
        uint256 amountTslaInUsdc = getUsdcValueOfUsd(getUsdValueOfGoog(amount));
    ...
    function _mintFulFillRequest(bytes32 requestId, bytes memory response) internal {
        ...
        if (amountOfTokensToMint != 0) {
            uint256 tokensInUSDC = getUSDCValueofUSD(getUsdValueOfGoog(amountOfTokensToMint));
    ...
    function _redeemFulFillRequest(bytes32 requestId, bytes memory response) internal {
        ...
        if (amountTokensWithdraw != 0) {
            uint256 tokensInUSDC = getUSDCValueofUSD(getUsdValueOfGoog(amountTokensWithdraw));
    ...
    function getGoogPrice() public view returns (uint256) {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(i_googUsdFeed);
    ...
    function getUsdValueOfGoog(uint256 tslaAmount) public view returns (uint256) {
    ...
    function getCalculatedNewTotalValue(uint256 addedNumberOfTsla) public view returns (uint256) {
        return ((totalSupply() + addedNumberOfTsla) * getGoogPrice()) / PRECISION;
    }
